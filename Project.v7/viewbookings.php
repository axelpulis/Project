...
<html lang="en">
    <head>
        ...
        <title>ShaveCave - View Appointments</title>
    </head>
...
WHERE client_id = $client_id AND booking.start_date >= '$currentDateTime' ORDER BY booking.start_date ASC";
        ...
        $counter=1;
           while ($row = mysqli_fetch_assoc($result)){
            if ($counter==1)
            { echo "<tr style='background-color:#ad8446;'>";
              $counter=0; 
            }
            else
            {
            ...
            <td>$row[type]</td> ";
            $string_duration = date('i \m\i\n\u\t\e\s',strtotime($row['Duration'])); echo"
            <td>$string_duration</td> 
            <td>$row[name]</td>
    ...
    <h1 class="display-4">Previous Appointments</h1><br>   
    <table class="table table-hover table-dark">
    ...
    <tr>
      ...
      <th scope="col"></th>
       ...
       <tbody>
        ...
        $get_future_bookings = "SELECT booking.booking_id, booking.start_date, barber.name, service.type ,service.price, service.Duration 
FROM booking INNER JOIN barber ON booking.barber_id = barber.barber_id INNER JOIN service ON booking.service_id = service.service_id
WHERE client_id = $client_id AND booking.start_date < '$currentDateTime'ORDER BY booking.start_date DESC";
